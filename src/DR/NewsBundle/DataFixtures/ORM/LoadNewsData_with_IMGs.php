...
~Mamie~
		...
		$order++;
			...
			$newsContainer[$num] = new News();
			$newsContainer[$num]
			->setTitle('Oour first mission: to welcome you')
			->setCategory($cat)->setDate(clone $time)
			->setAuthor('admin')->setPublished(true)->setImage(null)
			->setContent('
...
From Charles de Gaulle Airport by RER:
You will need to take the RER B to Châtelet and then the RER D from Châtelet to Évry
Duration: Two hours
Price: one way 18 €

From Orly Airport by RER and bus
You will need to take the RER and a bus, it will take you one hour to get to the airport:
Bus 285 to Juvisy. RER D from Juvisy to Évry Courcouronnes
Price: one way 5€

If you want to check the fastest way to get to your destination through public transport in the Paris agglomeration, you can always use this RATP website tool
					 <a href="http://www.ratp.fr/itineraires/en/ratp/recherche-avancee">Here</a>
					')
			->setOrdernumber($order);
		$order++;
			$time->add($t_add);//add one second each time for correct display order of the news
